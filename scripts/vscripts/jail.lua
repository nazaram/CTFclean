...
function BountyTriggered1a(trigger)
  if trigger.activator and trigger.activator:GetTeamNumber() == DOTA_TEAM_GOODGUYS then
    FindClearSpaceForUnit(trigger.activator, vector(-1876.36, 2654.45, 440.33), false)
    ...
    --trigger.activator:AddExperience(15 + 5 * _G.GAME_ROUND , DOTA_ModifyXP_Unspecified, false, false)
    ...
    local point =  Entities:FindByName( nil, "shield1" ):GetAbsOrigin()
...
function BountyTriggered1b(trigger)
  if trigger.activator and trigger.activator:GetTeamNumber() == DOTA_TEAM_GOODGUYS then
    print("potato1")
    FindClearSpaceForUnit(trigger.activator, vector(-1948.13, 2654.2, 472.407), false)
    --local point1 = Entities:FindByName( nil, "point_teleport_spot1" ):GetAbsOrigin()
    --FindClearSpaceForUnit(trigger.activator, point1, false)
    --trigger.activator:Stop()
    --SendToConsole("dota_camera_center")
...
function BountyTriggered2(trigger)
  if trigger.activator and trigger.activator:GetTeamNumber() == DOTA_TEAM_GOODGUYS then
    print("potato2")
    FindClearSpaceForUnit(trigger.activator, vector(-3249.98, 3185.92, 224.729), false)
...
function BountyTriggered3(trigger)
  if trigger.activator and trigger.activator:GetTeamNumber() == DOTA_TEAM_GOODGUYS then
    print("potato3")
    FindClearSpaceForUnit(trigger.activator, vector(-1971.92, -279.048, 249.174), false)
...
function BountyTriggered4a(trigger)
  if trigger.activator and trigger.activator:GetTeamNumber() == DOTA_TEAM_BADGUYS then
    FindClearSpaceForUnit(trigger.activator, vector(2373.58, 2674.95, 386.988), false)
    --print("in trigger 4")
    ...
    --trigger.activator:AddExperience(15 + 5 * _G.GAME_ROUND, DOTA_ModifyXP_Unspecified, false, false)
    --EmitGlobalSound( "Hero_Invoker.SunStrike.Ignite" )
    --MinimapEvent( DOTA_TEAM_GOODGUYS, entity, location.x, location.y, pingType, duration )
    --local bounty4 = Entities:FindByName(nil, "BountyBox4")
...
function BountyTriggered4b(trigger)
  if trigger.activator and trigger.activator:GetTeamNumber() == DOTA_TEAM_BADGUYS then
    FindClearSpaceForUnit(trigger.activator, vector(2624.12, 2674.78, 386.988), false)
  end
end
...
function BountyTriggered5(trigger)
  if trigger.activator and trigger.activator:GetTeamNumber() == DOTA_TEAM_BADGUYS then
      FindClearSpaceForUnit(trigger.activator, vector(3910.53, 3221.5, 244.969), false)
    --print("in trigger 5")
...
function BountyTriggered6(trigger)
  if trigger.activator and trigger.activator:GetTeamNumber() == DOTA_TEAM_BADGUYS then
    FindClearSpaceForUnit(trigger.activator, vector(2623.2, -253.662, 236.233), false)
  end
end
